...
class GameEngineAdapter:
    ...
    def serialize_board(board: Board) -> dict:
        try:
                ...
                ships_data.append(
                    {
                        "name": ship.name,
                        "coordinates": [
                            format_coordinate(c) for c in ship.coordinates
                        ],
            ...
            return {
                "ships": ships_data,
                "shots_received": [
                    format_coordinate(c) for c in board.shots_received
                ],
            }
        ...
        try:
            ...
            return {
                "player_board": {
                    ...
                    "shots_received": [
                        format_coordinate(c)
                        for c in engine.player_board.shots_received
                    ],
                },
                "ai_board": {
                    "ships": enemy_ships,
    ...
    @staticmethod
    def serialize_multiplayer_view(
        engine: GameEngine, player_slot: str
    ) -> dict:
        try:
            ...
            return {
                ...
                "my_ships_remaining": sum(
                    1 for s in my_board.ships if not s.is_sunk()
                ),
            ...
            raise RuntimeError(
                f"Failed to serialize multiplayer view: {e}"
            ) from e
